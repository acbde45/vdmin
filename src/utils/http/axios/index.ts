...
const transform: AxiosTransform = {
  ...
  transformRequestData: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    switch (code) {
      ...
      case ResultEnum.TIMEOUT:
        const LoginName = PageEnum.LOGIN_PAGE_NAME;
        const LoginPath = PageEnum.LOGIN_PAGE_PATH;
  ...
  requestInterceptors: (config, options) => {
    ...
    if (token && (config as Pojo)?.requestOptions?.withToken !== false) {
      // jwt token
      (config as Pojo).headers.Authorization = options.authenticationScheme
